...
async function main() {
  ...
  const contract = await ethers.getContractFactory("EIP4337Manager");
  const connected = contract.connect(relaySigner);

  const ret = await connected.deploy(ENTRYPOINT_ADDRESS);

  console.log("==EIP4337Manager addr=", ret.address);
}

// npx hardhat run scripts/account-abstraction/deploy/manager.ts --network <CHAIN_NETWORK>
main()
  .then(() => process.exit(0))
  .catch((err) => {
    console.error(err);
